...
Задания
    ...
    Добавьте в программу возможность измерения ещё одного аналогового канала, например, Ch 7 ADC 1.
...
// по заданию
#define ADC_7CHANNEL                 ADC_CHANNEL_7
...
// по заданию
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc_handle, ADC_7CHANNEL, &config));
    ...
    // по заданию
    adc_cali_handle_t adc_calibration_7handle = NULL;
    bool do_7calibration = adc_calibration_init(ADC_UNIT, ADC_7CHANNEL, ADC_ATTENUATION, &adc_calibration_7handle);
    ...
    while (1) {
        ...
        // по заданию
        ESP_ERROR_CHECK(adc_oneshot_read(adc_handle, ADC_7CHANNEL, &adc_7raw));
        printf("ADC%d Channel[%d] Raw Data: %d\n", ADC_UNIT + 1, ADC_7CHANNEL, adc_7raw);
        ...
        {
            // Иначе вывести расчетное значение в вольтах
            double voltage_calc = adc_raw > 3000 ? (0.0005 * adc_raw + 1.0874) : (0.0008 * adc_raw + 0.1372);
            printf("ADC%d Channel[%d] Calculated Voltage: %f mV\n", ADC_UNIT + 1, ADC_CHANNEL, voltage_calc);
        
        }

        // по заданию
        if (do_7calibration) {
            ESP_ERROR_CHECK(adc_cali_raw_to_voltage(adc_calibration_7handle, adc_7raw, &voltage_7));
            printf("ADC%d Channel[%d] Calibrated Voltage: %d mV\n", ADC_UNIT + 1, ADC_7CHANNEL, voltage_7);
        }
        else
        {
            // Иначе вывести расчетное значение в вольтах
            double voltage_calc = adc_7raw > 3000 ? (0.0005 * adc_7raw + 1.0874) : (0.0008 * adc_7raw + 0.1372);
            printf("ADC%d Channel[%d] Calculated Voltage: %f mV\n", ADC_UNIT + 1, ADC_7CHANNEL, voltage_calc);        
        }

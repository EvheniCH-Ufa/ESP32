...
void app_main(void)
{
    ...
    while (1) {
        ...
        if (do_calibration) {
            ...
            #define TDS_VOLTAGE_THRESHOLD 0.1  // Вольт

            if (sensor_voltage < TDS_VOLTAGE_THRESHOLD) // воздух дает 60 ppm ~ 0,1 вольта - игнорим их]
            {
                sensor_tds = 0;
                printf("Sensor is in AIR! TDS = 0 ppm\n");
            } 
            else
            {
        ...
        {
            // Иначе вывести расчетное значение в вольтах от потенциометра
            double voltage_calc = adc_raw *(3.3 / 4096);
            sensor_tds = (133.42 * voltage_calc * voltage_calc *voltage_calc - 255.86 * voltage_calc *voltage_calc + 857.39 * voltage_calc)*0.5;
            printf("TDS Calc = %.1f ppm\n", sensor_tds);
        }
